...
class PawnMovesCalculator implements MovesCalculator {
    ...
    private void tryCaptureMoves(int i, int j) {
        if (inBounds(i, j - 1) && !isEmpty(i, j - 1, board) && canCapture(i, j - 1, board, myPosition)) {
            addMove(i, j - 1);
        }
        if (inBounds(i, j + 1) && !isEmpty(i, j + 1, board) && canCapture(i, j + 1, board, myPosition)) {
            addMove(i, j + 1);
    ...
    private boolean validMove(int i, int j) {
        if (inBounds(i, j) && isEmpty(i, j, board)) {
            return true;
        }
        return false;
    }
